...
# from quickstart import main as upload_to_google_drive
# # Use the quickstart.py in the samefolder


# # Get the path of the directory where the current script (crawler.py) is located
# current_dir = os.path.dirname(os.path.abspath(__file__))
# print('current_dir:', current_dir)
# # Construct the path to quickstart.py located in the desired location
# quickstart_path = os.path.join(current_dir, 'quickstart.py')
# print('quickstart_path:', quickstart_path)
# # Add the path to quickstart.py to sys.path
# sys.path.append(os.path.dirname(quickstart_path))

# Import the main function from quickstart.py
...
def get_all_binance(symbol, kline_size, save=True, client=Client()):
    ...
    oldest_point, newest_point = minutes_of_new_data(
        symbol, kline_size, data_df, source="binance", client=client)
    ...
    if save and os.path.exists('./history'):
        ...
        # Upload to Google Drive using the imported function
    print('All caught up..!')
...
def get_nbars_binance(symbol, interval, nbars, client=Client()):
    ...
    """
    def interval_to_seconds(interval): return int(
        interval[:-1]) * {'m': 60, 'h': 60 * 60, 'd': 60 * 60 * 24}[interval[-1]]

    # calculate crawl time interval
    now = datetime.now(tz=timezone.utc)
    crawl_period = timedelta(seconds=interval_to_seconds(
        interval) * nbars) + timedelta(days=1)
    test_time = (now - crawl_period).strftime('%d %b %Y')

    # download results
    klines = client.get_historical_klines(
        symbol, interval, start_str=test_time)
    data = pd.DataFrame(klines,
...
def get_all_bitmex(symbol, kline_size, save=True, client=None):
    ...
    if os.path.isfile(filename):
        ...
        data_df = pd.DataFrame()
    oldest_point, newest_point = minutes_of_new_data(
        symbol, kline_size, data_df, source="bitmex", client=client)
    delta_min = (newest_point - oldest_point).total_seconds() / 60
    available_data = math.ceil(delta_min / binsizes[kline_size])
...
def get_all_google_drive(symbol, kline_size, save=True):
    """Getting historical price data from Google Drive.

    Args:
      symbol (str): Trading pair (ex: BTCUSDT).
      kline_size (str): A frequency of the price data (ex: "1m", "5m",'15m', '30m', "1h", '2h', "4h", "1d").
      folder_path (str): Path to the Google Drive folder containing historical data.
      save (bool): Save the results in ./history/ to improve the retrieve waiting time.
    ...
    if save and os.path.exists(folder_path):
