...
def plot_strategy(ohlcv, entries, exits, portfolio ,fig_data, html=None, k_colors='world'):
    ...
    c, info = chart.chart(ohlcv, overlaps=overlaps,
                          ...
                          start_date=ohlcv.index[-min(10000, len(ohlcv))], end_date=ohlcv.index[-1], k_colors=k_colors)
    c.load_javascript()
    if html is not None:
        c.render(html)
    else:
        ...
        with out:
            ...
            if name1 != name2:
                df = (getattr(portfolio, performance)()
                      ...
                      .reset_index().pivot_table(index=name1, columns=name2, values=performance))
