...
from finlab_crypto.crawler import get_nbars_binance, get_all_binance, get_all_google_drive
...
class TradingPortfolio():
    ...
    def get_full_ohlcvs(self):
        ...
        for (symbol, freq), lookback in symbol_lookbacks.items():
            try:
                ...
                print(f"Error retrieving data for {symbol}-{freq} from Binance: {e}")
                print("Trying to retrieve data from Google Drive...")
                try:
                    ohlcvs[(symbol, freq)] = get_all_google_drive(symbol, freq)
                except Exception as e:
                    print(f"Error retrieving data for {symbol}-{freq} from Google Drive: {e}")
    ...
    def calculate_position_size(self, signals, rebalance_threshold=0.03, excluded_assets=list()):
        ...
        # refine asset index
        # print("base_asset_value index shape:", base_asset_value.index.shape)
        # print("quote_asset_value index shape:", quote_asset_value.index.shape)
        # print("position index shape:", position.index.shape)
        ...
        transaction_btc = increase_asset_amount._append(decrease_asset_amount)
        ...
        transaction_btc = transaction_btc._append(pd.Series(txn_btc))
    ...
    def portfolio_backtest(self, ohlcvs, min_freq, quote_assets=['BTC', 'USDT', 'BUSD', 'USDC'], fee=0.002, delay=0):
        ...
        # Create a boolean mask for rows based on your conditions
        mask = (position != position.shift()).abs().sum(axis=1) != 0

        # Add the last timestamp to the mask
        last_timestamp = position.index[-1]
        mask.loc[last_timestamp] = True

        # Use the mask to index your DataFrame
